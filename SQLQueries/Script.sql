...
member_since datetime default '2020-07-18 20:28:08',
PRIMARY KEY (userID),
...
create index transactMerchandise on Transaction(mid);
...
SELECT A.username, COUNT(*)
  FROM Account A
       JOIN Transaction T
       ON A.userID = T.lender_userID
where not exists (select txl.userID , count(*)
					from LenderViewFromTransaction txl
                    where txl.userID = A.userID
					group by txl.userID
					having count(*)>1)
  GROUP BY A.username
  HAVING COUNT(*) > 1;

select * from  LenderViewFromTransaction;
...
select acc.userID, concat(acc.first_name,' ', acc.last_name) borrower_name, mer.category, mer.mid, 
		datediff(tx.due_date,tx.transaction_date) 'item_due in # days',
...
SELECT A.username, COUNT(*)
  FROM Account A
       JOIN Transaction T
       ON A.userID = T.lender_userID
where T.return_date is NULL and
	not exists (select txl.userID , count(*)
					from LenderViewFromTransaction txl
                    where txl.userID = A.userID
					group by txl.userID
					having count(*)>1)
...
-- ----------------------------------------------------------------------
-- Favorite Merchandises (Merchandises borrowed most number of times)
select price, sum(price) over() as price_sum
from Merchandise;


sum(count()) over() as count_val
from Transaction tx, Merchandise mer
where tx.mid = mer.mid;

select * from Merchandise where ownerID = 15700
insert into Transaction values (102291,  sysdate(),  DATE_ADD(sysdate() ,  INTERVAL 21 DAY), 15700, 10190,  26276, NULL);
insert into Transaction values (102292,  sysdate(),  DATE_ADD(sysdate() ,  INTERVAL 21 DAY), 20820, 10190,  26276, NULL);
insert into Transaction values (102293,  sysdate(),  DATE_ADD(sysdate() ,  INTERVAL 21 DAY), 15700, 11292,  10691, NULL);
insert into Transaction values (102294,  sysdate(),  DATE_ADD(sysdate() ,  INTERVAL 21 DAY), 20820, 11292,  10691, NULL);
